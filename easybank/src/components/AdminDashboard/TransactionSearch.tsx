...
const TransactionSearch = () => {
	...
	const translateType = (type: string) => {
		switch (type) {
			...
			case 'SENDER': return 'Enviada';
			case 'RECEIVER': return 'Recibida';
			default: return type;
		}
	};

	const hasSent = transactions.filter(t => t.type === "SENDER").length > 0;
	const hasReceived = transactions.filter(t => t.type === "RECEIVER").length > 0;
	...
	return (
		<div className="w-full h-full px-6 py-8 bg-gray-50 rounded-2xl shadow-sm">
			...
			{(hasSent || hasReceived) ? (
				<>
					{/* Tabla de transacciones enviadas */}
					{hasSent && (
						<div className="bg-white rounded-xl shadow border border-gray-200 overflow-hidden mb-8">
							<h2 className="px-6 pt-6 pb-2 text-lg font-semibold text-blue-700">Transacciones Enviadas</h2>
							<div className="overflow-x-auto">
								<table className="min-w-full divide-y divide-gray-200">
									...
									<tbody className="bg-white divide-y divide-gray-200">
										{transactions.filter(t => t.type === "SENDER").map((transaction) => (
					...
					{hasReceived && (
						<div className="bg-white rounded-xl shadow border border-gray-200 overflow-hidden">
							<h2 className="px-6 pt-6 pb-2 text-lg font-semibold text-green-700">Transacciones Recibidas</h2>
							<div className="overflow-x-auto">
								<table className="min-w-full divide-y divide-gray-200">
									<thead className="bg-gray-50">
										<tr>
									...
									<tbody className="bg-white divide-y divide-gray-200">
										{transactions.filter(t => t.type === "RECEIVER").map((transaction) => (
											<tr key={transaction.id} className="hover:bg-gray-50">
												<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{transaction.id.slice(0, 8)}...</td>
												<td className="px-6 py-4 whitespace-nowrap">
