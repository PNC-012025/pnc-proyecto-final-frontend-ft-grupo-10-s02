		...
		<div className="w-full h-full px-6 py-8 bg-gray-50 rounded-2xl shadow-sm">
			...
			{searched && !loading && bills.length === 0 && !error && (
				<div className="py-12 text-center">
					...
					<p className="text-gray-500 mt-1">Verifica el ID o intenta con otro usuario</p>
				</div>
			)}
						...
						<tbody className="bg-white divide-y divide-gray-200">
							{bills.map((bill) => (
								<tr key={bill.id} className="hover:bg-gray-50">
									<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{bill.id.slice(0, 8)}...</td>
									<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{bill.category}</td>
									<td className="px-6 py-4 whitespace-nowrap text-sm text-blue-800 font-semibold">${Number(bill.amount).toFixed(2)}</td>
									<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{bill.date ? new Date(bill.date).toLocaleDateString() : ''}</td>
									<td className="px-6 py-4 whitespace-nowrap text-sm">
										<span className={`px-3 py-1 rounded-full text-xs font-medium ${bill.state === "PENDING" ? "bg-yellow-100 text-yellow-800" : bill.state === "PAID" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-700"}`}>
											{bill.state === "PENDING" ? "Pendiente" : bill.state === "PAID" ? "Pagada" : bill.state}
										</span>
									</td>
								</tr>
							))}
